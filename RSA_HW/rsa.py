...
def encrypt(key, plaintext):
    ...
    blocksize = (x-1)//8
    plainBlocks = []
    quotient = asciilength//blocksize
    remainder = asciilength%blocksize




    for i in range(0,blocksize*quotient,blocksize):
     str1 = ""
     str1 = plaintext[i:i+blocksize]
     str_int = convertFromASCII(str1)
     plainBlocks.append(str_int)

    if(remainder != 0):
      str2 = ""
      str2 = plaintext[asciilength-remainder:asciilength]
      str_int2 = convertFromASCII(str2)
      plainBlocks.append(str_int2)
    ...
    ciphertext1 = ciphertext(cipherBlocks, asciilength,x)
    return ciphertext1
...
def decrypt(key, cipherText):
	cipherBlocks = cipherText.c
	retString = ""
	for block in cipherBlocks:
		plain = pow(block, key.d, key.n)
		plain2 = convertToASCII(plain)
		retString += plain2

	return retString# Given the passed rsakey, which will not have a private (d) key, it will
...
def sign(key, plaintext):
    hash_value = hashlib.sha256(bytes(plaintext,'ascii')).hexdigest()
    temp = key.d
    key.d = key.e
    key.e = temp
    encrypted = encrypt(key,hash_value)
    return encrypted
...
def checkSign(key, plaintext, signature):
    ...
    key.d = key.e
    key.e = temp
    hash_val = hashlib.sha256(bytes(plaintext,'ascii')).hexdigest()
    decrypted = decrypt(key, signature)
    return decrypted == hash_val
